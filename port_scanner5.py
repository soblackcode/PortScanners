...
from datetime import datetime # импорт модуля datetime из библиотеки datetime
...
def check_host(host): # Функция проверки адреса
	try: # пробуем из адреса вывести ip, если ошибки нет, то возвращаем ip
		ip = socket.gethostbyname(host)
		return ip
	except socket.error: # Если ошибка, то возвращаем False
		return False
...
def main(): # главная функция
	...
	if mode == '1': # если пользователь ввёл 1, то...
		...
		if not(check_host(host)): # Если функция вернула False, то...
			print(Fore.RED + '[!] ERROR: Wrong Host!') # Вывод о ошибке
			print(Style.RESET_ALL) # Сброс цветов
			exit(1) # Выход
		else: # Если вернула не False, то...
			ip = check_host(host) # Присваиваем адрес к переменной ip
			host = ip # host становится ip
		port = int(input(Fore.YELLOW + '\n[*] Enter Port: ')) # запрос порта у пользователя(желтый цвет)
		print(Fore.YELLOW + '\n[*] Scanning...\n') # (желтый цвет)
		start = datetime.now() # получаем время начала сканирования
		single_scanning(host, port) # запуск функции сканирования одного порта
		ends = datetime.now() # получаем время конца сканирования
		print(Fore.YELLOW + '\n[*] Time: ' + str(ends-start)) # Выводим время сканирования
		...
		if not(check_host(host)): # Если функция вернула False, то..
			...
			print(Style.RESET_ALL) # Сброс цветов
			exit(1) # Выход
		else: # Если она выдала результат, то...
			ip = check_host(host) # Присваиваем адрес к переменной ip
			host = ip # host становится ip
		...
		for port in ports: # перебор всех портов из словаря ports
			thread = threading.Thread(target=all_scanning, args=(host, port)) # создание потока, цель которого - функция "all_scanning", с аргументами - host, port
